...
# following function is equal to:
#[recipe.set_image() for recipe in recipes if first time trying and image does not exist already]
    missing_images = []
...
class RecipeBuffer:
    def __init__(self,data, buffer_size):
        ...
        self.buffer_index = 0
        initialize_all(self.data)    #SHOULD THIS GO HERE OR BELOW??? (precomputation)
    def current(self):
        output = []
        ...
        # initialize_all(output)       # SHOULD THIS GO HERE OR ABOVE??? (as-needed computation)
        return output
    def next(self): #currentbuffer+=8
        ...
        return self.current()
    def previous(self): #currentbuffer-=8
        new_index = self.buffer_index - self.buffer_size
        if(new_index<0):
            raise IndexError("RecipeBuffer has no previous")
        self.buffer_index = new_index
        return self.current()
    def first(self): #currentbuffer = 0 
        self.buffer_index = 0
        return self.current()
    def last(self): #currentbuffer = -1
